...
constexpr const char* SmokeInputImagePath = "/sdcard/Android/data/com.magicsr.uesmoke/files/MagicSRSmoke/input_64x64.pgm";
constexpr const char* SmokeOutputImagePath = "/sdcard/Android/data/com.magicsr.uesmoke/files/MagicSRSmoke/output_128x128.pgm";
...
bool WriteGrayPgm(const char* Path, const uint8* Data, int32 Width, int32 Height)
{
...
{
    for (int32 Attempt = 0; Attempt < 30 && !FileExists(SmokeModelPath); ++Attempt)
    {
        sleep(1);
    }

    if (!FileExists(SmokeModelPath))
    {
        return nullptr;
    }
    ...
    FCStringAnsi::Strncpy(Params.model_path, SmokeModelPath, sizeof(Params.model_path) - 1);
    Params.width = SmokeInputWidth;
    Params.height = SmokeInputHeight;
    ...
    if (Handle == nullptr)
    {
        __android_log_print(ANDROID_LOG_ERROR, "MagicSRUESmoke", "[MagicSRUESmoke] result=FAIL step=create model=%s", SmokeModelPath);
        return nullptr;
    }
    ...
    TArray<uint8> Input;
    Input.SetNumUninitialized(SmokeInputWidth * SmokeInputHeight);
    for (int32 Y = 0; Y < SmokeInputHeight; ++Y)
    {
        for (int32 X = 0; X < SmokeInputWidth; ++X)
        {
            Input[Y * SmokeInputWidth + X] = static_cast<uint8>((X * 3 + Y * 5) & 0xff);
        }
    }

    TArray<uint8> Output;
    Output.SetNumZeroed(SmokeInputWidth * SmokeScale * SmokeInputHeight * SmokeScale);
    const int32 ProcessRet = MC_Process(Handle, Input.GetData(), Output.GetData());
    mkdir(SmokeOutputDir, 0775);
    const bool bWroteInput = WriteGrayPgm(SmokeInputImagePath, Input.GetData(), SmokeInputWidth, SmokeInputHeight);
    const bool bWroteOutput =
        WriteGrayPgm(SmokeOutputImagePath, Output.GetData(), SmokeInputWidth * SmokeScale, SmokeInputHeight * SmokeScale);
    int32 NonZero = 0;
    ...
    __android_log_print(bPass ? ANDROID_LOG_INFO : ANDROID_LOG_ERROR,
                        "MagicSRUESmoke",
